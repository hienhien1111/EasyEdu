...
const STUDENT = {
  username: 'qr.split1',
  email: 'qr.split1@student.edu.vn',
  phone: '0902999021',
  fullName: 'QR Split Test Student',
};

const TEST_CLASSES = [
  {
    name: 'QR Split Test - Toan 2K',
    subject: 'Toan',
    tuitionPerSession: 2_000,
    statuses: ['PRESENT', 'ABSENT_UNEXCUSED'],
  },
  {
    name: 'QR Split Test - Anh 2K',
    subject: 'Anh',
    tuitionPerSession: 2_000,
    statuses: ['PRESENT'],
  },
...
function historyPeriod() {
  ...
  if (yesterday >= current.start) {
    return {
      ...current,
      label: `QR split test ${current.label}`,
      end: yesterday,
    };
  }
  ...
  return {
    ...previous,
    label: `QR split test ${previous.label}`,
  };
}
  ...
  for (const input of TEST_CLASSES) {
    const cls = await prisma.class.upsert({
      ...
      update: {
        ...
        maxStudents: 30,
        isActive: true,
        description: 'Lop hoc phi thap de test QR nhieu lop.',
      },
      create: {
        name: input.name,
        subject: input.subject,
...
async function clearOldInvoices(studentId: string) {
  const oldInvoices = await prisma.invoice.findMany({
    where: {
      studentId,
      OR: [
        { periodLabel: { startsWith: 'QR split test' } },
    ...
    for (const [index, status] of cls.statuses.entries()) {
      ...
      await prisma.attendance.create({
        data: {
          ...
          isLocked: true,
          note: 'Seed QR split billing test',
        },
      });
    }
  }
...
async function main() {
  console.log('Seeding QR split invoice test data...');
  ...
  await clearOldInvoices(student.id);
  ...
  console.log(`Max successful QR payments: ${invoice.maxPaymentTimes}`);
